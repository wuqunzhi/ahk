...
setup() {
    ...
    ; flag: ^ 必须全匹配
    addCommand(id, func := id, funcArg := "", hint := id, flag := "") {
        cmds.Set(id, [func, funcArg, hint, flag])
    }
    ...
    addCommand("^q main", "runAs", "main.ahk", "runAs main.ahk")
    ...
    if (input != text) {
        ...
        for k, v in cmds {
            ...
            match(k, text) {
                ; k      =   ^q main
                ; text   =   q m
                ; https://wyagd001.github.io/v2/docs/lib/InStr.htm
                if (SubStr(k, 1, 1) == '^') { ; 必须开头处
                    return InStr(k, text, true) == 2
                }
                else if (SubStr(k, -1) == '$') { ;必须最后匹配
                    return InStr(k, text, true) == StrLen(k) - StrLen(text)
                }
                else {
